...
document.addEventListener("keyup", (pressed) => {
	...
	if (
		code == "Enter" &&
		!input_score.classList.contains("btn-outline-warning")
	) {
		buttonPressed("okay");
	}
});
...
for (let i = 0; i < buttons.length; i++) {
	buttons[i].addEventListener("click", function () {
		if (
			!buttons[i].classList.contains("hotkey") &&
			input_score.innerHTML.trim() != "0" &&
			buttons[i].value != "togo" &&
			buttons[i].value != "undo" &&
			buttons[i].value != "okay"
		) {
	...
	switch (key) {
		case "okay":
			...
			player_last_score.innerHTML =
				player_last_scored[player_last_scored.length - 1];
			input_score.innerHTML = 0;
		...
		case "hotkey":
			old_player_score.push(parseInt(player_score.innerHTML));
			player_score.innerHTML =
				parseInt(player_score.innerHTML) - parseInt(value);
...
function undoScore() {
	...
	if (old_player_score.length > 0) {
		...
		if (
			player_rounds_leg[parseInt(player_legs_score.innerHTML)] == 0 &&
			player_rounds != 0
		) {
			player_leg_average = 0;
...
function calculateAverage(key) {
	...
	if (player_rounds_leg[parseInt(player_legs_score.innerHTML)] != 0) {
		player_leg_average =
			(501 - player_score.innerHTML) /
			player_rounds_leg[parseInt(player_legs_score.innerHTML)];
	}
	if (player_rounds != 0) {
		player_overall_average = player_scored / player_rounds;
	}
	player_average_leg.innerHTML = parseFloat(player_leg_average).toFixed(2);
	player_average_overall.innerHTML = parseFloat(player_overall_average).toFixed(
		2
	);
}
